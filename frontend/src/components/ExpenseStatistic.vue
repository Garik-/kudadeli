...
<template>
  ...
  <div class="flex flex-wrap gap-2 mb-6">
    <div v-for="category in expensesStore.groupedByCategory" :key="category.amount"
      :class="['flex items-center gap-1 rounded-full p-1', getLabelColor(category.color, 100)]">
      <div class="w-8 h-8 rounded-full p-2" :class="category.color">
        <component :is="category.icon" class="w-full h-full text-white" />
      </div>
      <div class="text-sm font-semibold text-gray-700 pr-2">{{ category.name }} {{ category.amountFormatted }}</div>
    </div>
  </div>
</template>
